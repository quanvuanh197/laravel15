	...
	<div class="col-md-12">
		...
		<div class="panel panel-default">
			<div class="panel-heading">
				<h2 class="panel-title"><strong>In Stock Products</strong></h2>

				<ul class="pull-right">
					<a type="button" class="btn btn-primary btn-add" title="Add New" brand-url="{{asset('/admin/products/brandsall')}}" size-url="{{asset('/admin/products/sizesall')}}" data-toggle="tooltip" id="post-detail" ><span class="fa fa-plus"></span></a>
					@include('/admin/products/store')
				</ul>
			</div>
			<div class="panel-body">

				<table class="table datatable" id="product-instock-table">
					<thead>
...
@section('script')
	<script type="text/javascript">
		...
		$(document).ready(function () {
			$('#product-instock-table tbody').on('click', '.btn-delete-product', function(event) {
			...
			$('.btn-add').click(function(){
				...
				$('#price_input').empty();
				$('#sale_price_input').empty();
				var url_brand=$(this).attr('brand-url');
				var url_size=$(this).attr('size-url');
				...
				$.ajax({
					...
					//nếu thực hiện thành công thì chạy vào success
					success: function (response) {
						$('#size_input_group').empty();
						$.each(response.sizes_data, function( key, value ) {
							$('#size_input_group').append( "<div class="+"col-md-4 "+"style="+"margin-top:2em"+"><div class="+"input-group"+"> <span class="+"input-group-addon"+"><span>Size "+value.size+"</span></span> <input name="+value.size+" type="+"number"+" class="+"form-control "+"placeholder="+"Enter"+"&nbsp;"+"Quantity"+" /> </div></div>" );
				...
				$('.btn-file-add :file').on('fileselect', function(event, label) {
			...
			$('#product-instock-table tbody').on('click', '.btn-show', function(event) {
				...
				$.ajax({
					...
					success: function (response) {
						...
						$('#detail-sale_price').text(response.data.product['sale_price_format']);
			...
			$('#product-instock-table tbody').on('click', '.btn-edit', function(event) {
				...
				$.ajax({
					...
					success: function (response) {
						...
						$('#price_input_update').text(price_format_edit+' VNĐ');
						...
						$('#sale_price_input_update').text(sale_price_format_edit+' VNĐ');
	...
	$('#product-instock-table tbody').on('click', '.btn-edit-image', function(event) {
	...
	$('#product-instock-table tbody').on('click', '.btn-edit-quantity', function(event) {
